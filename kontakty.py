...
def create_contacts(card_type, number_of_gen_cards):
    if card_type == "P":
        list_1 = []
        for i in range(number_of_gen_cards):
            list_1.append(BaseContact(name=fake.first_name(), surname=fake.last_name(), private_phone=fake.phone_number(), e_mail=fake.email()))
        return list_1
    else:
        list_1 = []
        for i in range(number_of_gen_cards):
            list_1.append(BusinessContact(name=fake.first_name(), surname=fake.last_name(), private_phone=fake.phone_number(), e_mail=fake.email(),
                                           position=fake.job(), company=fake.company(), company_phone = fake.phone_number()))
        return list_1
...
#Definiujemy funkcję w klasie która bedzie która w konsoli bedzie wyświetlała napis wymagany dla wizytówki biznesowej
    def contact(self):
...
if priv_or_busin == "P":
    base_card_list = create_contacts(priv_or_busin,numb_of_cards)
    print("Stworzono listę zawierającą prywatne wizytówki w liczbie: %d" % numb_of_cards)
else:
    business_card_list = create_contacts(priv_or_busin,numb_of_cards)
    print("Stworzono listę zawierającą biznesowe wizytówki w liczbie: %d" % numb_of_cards)
...
while True:
    while True:
#Uzytkownik definiuje jakączynność program ma wykonać
...
#W przypadku wybrania 1 program Ponumeruje i wyświetli dane wszystkich wizytówek w osobnych liniach 
    if activity == 1:
        if priv_or_busin == "P":
            for i in range(len(base_card_list)):   
                print("%d. " %(i+1), base_card_list[i].name, "  ", base_card_list[i].surname, "  ", base_card_list[i].private_phone, " ", base_card_list[i].e_mail)
        else:
            for i in range(len(business_card_list)):  
                print("%d. " %(i+1), business_card_list[i].name, "  ", business_card_list[i].surname, "  ", business_card_list[i].private_phone, " ", business_card_list[i].e_mail, " ",
                      business_card_list[i].position , business_card_list[i].company, business_card_list[i].company_phone)
...
#W przypadku wybrania 2 program "zadzwoni" do jednej osoby której numer podano w poprzedniej opcji, tj 1,2,3,4,5 itd
        ...
        while True:
            ...
            try:
                ...
                wrong_number()
        if priv_or_busin == "P":
            base_card_list[contact_number-1].contact()
        else:
            business_card_list[contact_number-1].contact()
...
#podczas opcji "1". Numer osoby to 1,2,3,4,5 itd 
        ...
        while True:
            ...
            try:
                ...
                wrong_number()
        if priv_or_busin == "P":
            print("Łączna długość imienia i nazwiska dla ", base_card_list[contact_number-1].name, base_card_list[contact_number-1].surname ,"to: ",base_card_list[contact_number-1].label_length)
        else:
            print("Łączna długość imienia i nazwiska dla ", business_card_list[contact_number-1].name, business_card_list[contact_number-1].surname ,"to: ",business_card_list[contact_number-1].label_length)
